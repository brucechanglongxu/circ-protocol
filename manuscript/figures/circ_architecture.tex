\begin{tikzpicture}[
  ...
  edgelabel/.style={fill=white, inner sep=1.2pt, font=\sffamily\tiny}
]
  ...
  \draw[flow] (external) -- node[above, sloped, edgelabel]
    {signed, scoped intent} (coord);
  \draw[check] (coord.south west) -- node[above, sloped, edgelabel] {state/action} (ehr.north);
  \draw[check] (coord.south) -- node[right, edgelabel] {verify} (identity.north);
  \draw[check] (coord.south east) -- node[above, sloped, edgelabel] {authorize} (policy.north);
  \draw[flow] (coord.west) -- ++(-1.7,0) |- node[pos=0.82, below, edgelabel] {append} (audit.west);
  \draw[escalation] (coord.east) -- ++(1.7,0) |- node[pos=0.82, below, edgelabel]
    {escalate} (supervisor.east);
  \draw[check] (external) -- (externaltrust);

  \begin{scope}[on background layer]
    \node[boundary, fit=(agent1)(agent2)(coord)(ehr)(identity)(policy)(audit)(supervisor),
